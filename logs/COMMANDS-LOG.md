...
- **Outcome:** Blocked by credential type.
  - Provided token is `<slack-app-token>` (app-level token), which cannot call `chat.postMessage`.
  - Need bot user token `<slack-bot-token>` with `chat:write` and bot added to `#demos`.
...
- **What:**
  1. Updated `dark-noc-mcp/slack-secrets` with provided `<slack-user-token>` token.
...
- **What:**
  1. Updated `dark-noc-mcp/slack-secrets` with provided bot token (`<slack-bot-token>`).
